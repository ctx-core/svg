...
export const set_svg_matrix2d_b:B<set_svg_matrix2d_T> = be_(key, ctx=>{
	...
	async function set_svg_matrix2d(
		...
		svg_margin$_b(ctx).$ = svg_margin
		svg_height$_b(ctx).$ = svg_height
		svg_width$_b(ctx).$ = svg_width
		svg_content_height$_b(ctx).$ = svg_content_height
		svg_content_width$_b(ctx).$ = svg_content_width
	}
})
